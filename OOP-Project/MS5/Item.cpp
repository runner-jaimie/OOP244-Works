    ...
    istream& Item::read(istream& istr)
    {
        ...
        do // validation한 값이 들어 올때까지
        {
            ...
            if(!istr || strlen(tempSku) > MAX_SKU_LEN || strlen(tempSku) == 0 )// istr이 내가 입력한 데이타 타입을 확인해준다.
            {
                if(!istr)
                {
                    istr.clear(); // input stream이 fail 하면 clear함수 필요 (즉 fail bit를 지운다)
                    istr.ignore(1000, '\n');
                }
    ...
    ifstream& Item::load(ifstream&  ifst)
    {
        ...
        if(ok)
        {
            ...
            if(!ifst)
            {
                ifst.clear();
                ifst.ignore(1000,',');
                //m_error = ERROR_POS_NAME; all error message는 나중에 validation에서 체크 해주고 에러 메세지 셋팅을 해주니깐 지금 파일 읽어 올땐 필요없다. 
                ok = false;
            }
        }
